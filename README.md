...
- **Hero roster (Phase 6)** — the full 30-hero roster across 5 rarities, a Hero screen to recruit, level up, and rank up heroes, and a 5-slot battle team. All gold/gem costs and level/rank caps are computed and enforced server-side; only *equipped* heroes feed into Kingdom Power, so the team you build there is the team that fights.
...
/server            Node + TypeScript + Express backend
  /src/routes      auth, kingdom, battle, hero
  /src/services    kingdomService (economy, upgrades, power), battleService (combat resolution), heroService (recruit/level/rank/equip)
  /src/telegram    initData verification, bot.ts (long-polling bot)
  /src/middleware  auth (Telegram verification), errorHandler
  /src/config      gameConfig.ts — building costs, battle stages, hero rarity/level/rank economy (admin-editable later)
...
npm run seed             # loads the full 30-hero roster
```
...
- Hero recruit/level-up/rank-up costs, level caps, and rank caps are all computed and enforced server-side (`gameConfig.ts` + `heroService.ts`), with gold/gem deduction inside the same Prisma transaction as the stat change — the client can request an action but can't choose its price or bypass a cap.
